...
//KenkyuKadai1が一応完成形なので、そちらをご参照ください。
public class KadaiMain {
	public static void main(String[] args) {

		//登録依頼ファイルのパスを指定
		String pass = "C:\\Users\\8_bra\\Desktop\\研修課題【初級】\\資料\\テストデータサンプル\\regist_T_SYAIN_MST.csv";
		//削除依頼ファイルのパスを指定
		String pass1 = "C:\\Users\\8_bra\\Desktop\\研修課題【初級】\\資料\\テストデータサンプル\\delete_T_SYAIN_MST.csv";
		...
		if (file.exists() == true && file2.exists() == true){
			...
			List<String> li1 = FileLoader.registLoader(file);
			//ファイル読み込みメソッドの呼び出し。引数として削除依頼ファイルを渡す。
			//","を削除した値を格納したリストを戻り値として受け取る。
			List<String> li2 = FileLoader.deleteLoader(file2);

			//登録依頼ファイル(リスト1)が詳細設計書のチェック内容に基いているか確認するメソッドを呼び出す。
			//戻り値として、チェック内容に基いていればtrueを、NG項目が一つでもあればfalseを受け取る。
			boolean result = FileChecker.registChecker(li1);

			//削除依頼ファイル(リスト2)が詳細設計書のチェック内容に基いているか確認するメソッドを呼び出す。
			//戻り値として、チェック内容に基いていればtrueを、NG項目が一つでもあればfalseを受け取る。
			boolean result2 = FileChecker.deleteChecker(li2);

			//登録依頼ファイルと削除依頼ファイルが両方ともチェック内容に基いていれば処理を続行する。
			if(result == true && result2 == true){

				//登録依頼ファイルから読み込んだデータを社員マスタテーブルに登録する。
				RegistAndDelete.masterRegist(li1);
				System.out.println("登録依頼ファイルのデータは正常に社員マスタテーブルに登録されました");

				//削除依頼ファイルから読み込んだデータと社員マスタテーブルの社員IDが一致する
				//データを社員マスタテーブルから削除する。
				RegistAndDelete.masterDelete(li2);
				System.out.println("削除依頼テーブル内で指定された社員IDと合致するレコードを社員マスタテーブルから削除しました");
